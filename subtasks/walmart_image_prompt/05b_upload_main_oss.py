"""Step 05b: Upload generated main images in the current batch to Aliyun OSS."""
...
from workflow_common import (
    UPLOAD_MAIN_CONFIG,
    apply_batch_to_main_oss_config,
    print_batch_info,
    task_execution,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="05b 上传主图 OSS")
    parser.add_argument("--dry-run", action="store_true", help="只预览主图 OSS 上传任务，不连接 OSS，不上传。")
    ...
    config = apply_batch_to_main_oss_config(load_stage_config(UPLOAD_MAIN_CONFIG, load_config), args.batch_name)
    ...
    print("\n=== 05b 汇总 ===")
    print(f"主图OSS上传: {len(records)} | 成功/跳过: {success_count} | 失败: {len(records) - success_count}")
