...
def main() -> None:
    ...
    print(
        ...
        f"查询未确定: {pending_count} | 明确失败: {failed_count} | "
        f"目标已满足跳过: {skipped_count} | 依赖未满足: {blocked_count}"
    )
...
def preview() -> None:
    ...
    print(f"候选输入: {len(rows)} 行 / {count_skus(rows)} 个 SKU")
    print(
        f"最终目标: {desired_result_count(rows, config.desired_count)} 张 | "
