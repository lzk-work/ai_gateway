...
图片 API 限速：提交、查询和下载分别由 `limits.submit_delay_seconds`、`limits.query_delay_seconds` 和 `limits.download_delay_seconds` 控制。三项当前均为 `0.5` 秒，表示同一工作线程内同类请求的最小启动间隔；不同并发线程互不等待。总体吞吐由图片并发数、线程间隔和接口响应时间共同决定。单任务查询及下载重试间隔仍由 `retry` 配置控制。
某张图片下载返回 HTTP 429 时，立即停止该图片本轮下载，不尝试其他 URL；保留原 task_id，下一轮继续查询下载，不重新生图、不增加重新生成次数。其他图片正常处理，不全局暂停。下载网络故障耗尽重试后同样保留 task_id，提示“下载未完成”。明确失效的图片仍按既有重新生成上限处理。
...
OSS 上传断点采用追加写，同一 `SKU + 图片命名` 读取最后一条状态，兼容已有断点。JPG 本地路径会在删除 PNG 前立即追加到生成 checkpoint，生成结果汇总每轮最多统一同步一次，避免大批次逐图全量重写 JSONL；中断续跑、已上传跳过和已有 task_id 规则保持不变。
