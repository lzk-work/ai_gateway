"""Step 02: Call the configured text-model gateway and write validated JSON results."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="02 调用文本模型网关")
    parser.add_argument("--dry-run", action="store_true", help="只预览待调用任务，不调用模型，不写结果。")
    parser.add_argument(
...
def preview(config, execution, preview_current_excel: bool = False) -> None:
    ...
    print(f"\n=== 02 调用 {(config.gateway or '默认').upper()} 文本模型 | 试运行 ===")
    print("试运行: 不调用模型，不写 JSONL/Excel")
