...
- 公共执行器负责并发、checkpoint、周期查询与下载；平台适配器负责请求构造、查询及响应解析。
...
新平台只接受明确成功且包含可下载图片地址的响应。TUZI 在首轮只批量提交并保存 task_id，后续按业务 scheduler 的可配置间隔重跑完整流程并查询下载；MXAPI 备用仍保持即时轮询。每次下载完成后继续使用既有结果格式，供 OSS 阶段消费。
